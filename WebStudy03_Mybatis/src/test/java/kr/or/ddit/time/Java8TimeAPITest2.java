...
// 테스트케이스
	...
	void test1() {
		...
		System.out.println(YearMonth.from(LocalDate.from(LocalDateTime.now())));
		System.out.println(Year.from(YearMonth.from(LocalDate.from(LocalDateTime.now()))));
	}
	...
	void test2() {
		...
		System.out.println(LocalDate.from(current).format(DateTimeFormatter.ofLocalizedDate(FormatStyle.FULL).withLocale(Locale.CANADA)));
	...
	void test5() {
		...
		Locale locale = Locale.forLanguageTag(Locale.GERMANY.toLanguageTag());
