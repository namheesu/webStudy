...
/**
 * 인증된 사용자(sessionscopeㅇ내의 authMember속성을 가진)의 경우
	...
	private static class GeneratePrincipalHttpServletRequestWrapper extends HttpServletRequestWrapper{
		...
		public Principal getUserPrincipal() {
			HttpSession session = getSession(false);
				if(session != null) {
					MemberVO authaMember = (MemberVO) session.getAttribute("authMember");
				if(authaMember != null) {
					MemberVOWrapper principal = new MemberVOWrapper(authaMember);
					return principal;
				}
			}
