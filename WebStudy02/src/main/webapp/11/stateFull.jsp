...
<pre>
	1. session : 상태 유지를 위한 정보가 서버측에 저장되는 개념. 세션을 통해 저장되는 데이터는 타입제한이 없다.
		세션(HttpSession )
			database -> connectFull --> connection == session, session을 통로의 의미로 사용
		...
		세션의 생명주기
			...
			유지 방법 : 서버와 클라이언트가 동일한 세션 아이디를 공유하는 상황. (tracking mode)
				...
				2) URL : request line을 통해 세션 아이디 공유(보안 취약함)
			...
			소멸 : 만료 시간(timeout)이내에 새로운 요청이 발생하지 않는 경우.
				1) 브라우저 종료
				2) 쿠키 삭제 
				3) 직접 로그아웃(session invalidate) : 쓰래기 세션 쌓이지 않음
				<%
//					session.invalidate();
				%>
	2. cookie : 상태 유지를 위한 정보가 클라이언트측에 저장되는 개념. 쿠키을 통해 저장되는 데이터는 String타입만 가능.
		1) 쿠키 객체 생성 : name/ualue속성, value에 한글을 비롯한 특수문자가 포함된 경우 -> URLencoding 과정이 필요함.
		2) 쿠키의 부가 속성 설정.
			- name/value (required)
			- domain : 생략한 경우, 쿠키를 생성할때 도메인이 반영됨.
						쿠키의 재전송 여부를 결정하는 조건.
						ex) www.naver.com : www.naver.com을 대상으로 한 요청에만 재전송.
								.naver.com : *.naver.com을 대상으로 한 요청에만 재전송.
			- path : 생략한 경우, 쿠키를 생성할때 경로가 반영됨.
					쿠키의 재전송 여부를 결정하는 조건.
					ex) / : 특정 도메인 이후의 모든 경로로 발생한 요청에 재전송됨.
						request.getContextPath() : 특정 도메인의 해당 컨텍스트로 발생한 요청에 재전송됨.
						request.getContextPath()/10 : 특정 도메인의 해당 컨텍스트로 "/10" 경로로 발생한 요청에 재전송됨.
			- maxAge : 쿠키의 저장 시한 결정(초단위).
					ex) 60 * 60 * 24 * 7
						0 : 쿠키 삭제  
					   -1 : 브라우저 종료 시 쿠키 삭제
			- httpOnly : 클라이언트 사이드 모듈에서의 접근을 허용하지 않을때.
			- secure : SSL(https) 프로토콜에서만 쿠키를 
		3) response.addCookie : 응답 헤더(set-cookie)를 통해 클라이언트로 전송.
		
		4) 음답에 포함된 쿠키를 부라우저의 쿠기 저장소에 저장 
		5) 다음번 요청에 다시 쿠키를 요청헤더(cookie)를 통해 재전송함.
...
// 			response.addCookie(firstCookie);
			...
			String firstCookieValue = Optional.ofNullable(request.getCookies())
											...
											.orElse(null);
 		%> 
		상태를 복원한 firstCookieValue 값 : <%=firstCookieValue %> 
</pre>
</body>
</html>