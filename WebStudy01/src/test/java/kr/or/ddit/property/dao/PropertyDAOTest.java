...
//		assertion
		assert propList != null;	// true아니면 false
	...
	void testSelectProperty() {
		for(int i=1; i<=2000; i++) {
			PropertyVO vo = dao.selectProperty("NLS_SORT");
			assertNotNull(vo);
		}
	}
	...
	void testUpdateProperty() {
		...
		modifyProp.setPropertyName("ajdkdkdk");
		modifyProp.setPropertyValue("modified value");
	...
	void testDeleteProperty() {
		int rowcnt = dao.deleteProperty("NLS_TIME_FORMAT");
		assertEquals(1, rowcnt); 	// 예측
	}
